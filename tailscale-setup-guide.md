# Dev Dock 啟用教學（Tailscale 可開啟版）

## 目標
讓你的 Dev Dock 管理台與啟動的 dev server，可在 **手機/平板（同一個 Tailscale 網路）** 上直接開啟。
...
## 2) 啟動 Dev Dock 管理台（Tailscale 可存取）
關鍵在於綁定 `0.0.0.0`：
...
## 6) 專案本身也要能被網路存取（關鍵）
Dev Dock 會幫你啟動時加入 `--host 0.0.0.0` 的參數（Vite / Next / Astro / Nuxt 常見框架皆已支援），
