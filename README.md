...
Dev Dock 是一套本機開發伺服器管理工具，專為需要同時管理多個 `npm run dev` 專案的 Windows 開發環境設計。它提供 Web 管理介面與 PowerShell CLI，可自動掃描含有 `package.json` 與 `scripts.dev` 的專案，集中啟動、停止、重啟、查看狀態與讀取 log，並支援透過 LAN 或 Tailscale 將開發中的網站分享給手機、平板或同一網路中的裝置測試。
...
- **狀態保存與自動復原**：執行狀態會保存在 `.dev-manager/state.json`，可在 Dev Dock 重啟後復原先前分享中的專案。
...
3. 按下「掃描」，Dev Dock 會找出可執行 `npm run dev` 的專案。
